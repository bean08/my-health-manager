...
APP_NAME="MyHealthManager"
DISPLAY_NAME="My Health Manager"
...
ICONSET_DIR="$(mktemp -d "${TMPDIR:-/tmp}/my-health-manager.iconset.XXXXXX")/AppIcon.iconset"
...
<dict>
  ...
  <string>io.github.my-health-manager</string>
...
rm -f "$DMG_PATH"
hdiutil create -volname "$DISPLAY_NAME" -srcfolder "$APP_DIR" -ov -format UDZO "$DMG_PATH"
